...
class ActionGetAvailableRooms(Action):
    def name(self) -> Text:
        return "action_get_available_rooms"
    # This function takes the checkin and checkout dates and the number of people traveling.
    # It returns the name of one room that is available on those dates.
    ...
    def run(
        ...
        number_of_customers = tracker.get_slot("amount_ppl")
        date_checkin = dateparser.parse(tracker.get_slot("checkin_date"), date_formats=['%Y-%m-%d'])
        date_checkout = dateparser.parse(tracker.get_slot("checkout_date"), date_formats=['%Y-%m-%d'])
...
class ActionMakeBooking(Action):
    ...
    def run(
        ...
        date_checkin = dateparser.parse(tracker.get_slot("checkin_date"), date_formats=['%Y-%m-%d'])
        date_checkout = dateparser.parse(tracker.get_slot("checkout_date"), date_formats=['%Y-%m-%d'])
        customer_wants_breakfast = tracker.get_slot("customer_wants_breakfast")
